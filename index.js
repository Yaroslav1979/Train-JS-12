...
function addKeyValuePairs(dictionary, entries) {
  ...
  let added = 0;
  let rejected = 0;
  entries.forEach(([key, value]) => {if (!dictionary.has(key)) {
    dictionary.set(key, value);
    added ++;
  } else {
    rejected ++;
...
function checkPresence(dictionary, keys) {
  let arrCheckPresence = [];
  keys.forEach(key => {let result = dictionary.has(key);
  arrCheckPresence.push(result)});
  return arrCheckPresence;
...
function sortByValues(dictionary) {
  const iteratorjhArr = dictionary.entries();
  const sortedArray = Array.from(iteratorjhArr);
  sortedArray.sort((a, b) => b[1] - a[1]);
  const sortedDictionary = new Map(sortedArray);
  ...
  // Метод сортування sortedArray.sort((a, b) => b[1] - a[1]) використовується для сортування масиву пар ключ-значення за значеннями в порядку спадання (від найбільшого до найменшого значення).

// Ось як він працює:

// sortedArray - це масив, який містить пари ключ-значення з вашого словника (Map). Кожен елемент sortedArray є масивом з двох елементів: перший елемент - це ключ, а другий - значення.

// sort - це метод масиву, який приймає функцію порівняння. Функція порівнювання отримує два елементи для порівняння: a та b.

// (a, b) => b[1] - a[1] - ця функція порівняння визначає, як слід порівнювати a та b для сортування. Вона використовує другий елемент кожного масиву (тобто значення) для порівняння.

// b[1] - a[1] - це вираз, який обчислює різницю між значеннями b та a. Якщо результат цієї різниці від'ємний, то b буде поміщено перед a у відсортованому масиві, тобто в порядку спадання. Якщо результат цієї різниці дорівнює нулю, то порядок елементів не змінюється, інакше, якщо результат є додатнім, то a буде поміщено перед b.

// Отже, в результаті використання цієї функції порівняння, масив sortedArray буде відсортований за значеннями в порядку спадання, де елементи з більшими значеннями будуть на початку масиву, а елементи з меншими значеннями - в кінці.
...
function convertDictionaryToSet(dictionary) {
  const setConvert = new Set();
  for (const [key] of dictionary.entries()) {
    if (typeof(key) === "string") {
      setConvert.add(key.toUpperCase())
    } else if  (typeof(key) === "number") {
      setConvert.add(key + 1)
    }
  }
  return setConvert
...
function convertSetToDictionary(set) {
  ...
  for (const element of set) {
    const key = element;
    const value = element.charAt(0);
    map.set(key, value.charCodeAt(0));
  ...
  // Використовуємо element.charAt(0) для отримання першого символу елемента, а потім визначила його ASCII-код за допомогою value.charCodeAt(0).
